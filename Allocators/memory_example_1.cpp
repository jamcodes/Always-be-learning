...
// alias namespace foonathan::memory as memory
...
int main()
{
    ...
    // allocate std::unique_ptr using the pool
    // memory::allocate_shared is also available
    auto ptr{memory::allocate_unique<int>(pool, list.front())};
    std::cout << *ptr << "\n";

    // static storage of 4KiB
    ...
    // again, just an alias for
    // std::unordered_set<int, std::hash<int>, std::equal_to<int>,memory::std_allocator<int, static_pool_t>
