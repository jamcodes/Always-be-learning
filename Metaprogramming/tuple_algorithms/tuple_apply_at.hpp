...
template <typename Tuple, typename Action, std::size_t I,
          bool = std::is_invocable_v<Action, decltype(std::get<I>(std::declval<Tuple>()))>>
struct tuple_apply_at_helper {
    constexpr void operator()(Tuple&&, Action&&) const
    {
        std::cerr << "\n" << __PRETTY_FUNCTION__ << "\n";
    }
};
...
struct tuple_apply_at_impl<std::index_sequence<I, Is...>> {
    ...
    {
        if (I == index) {
            tuple_apply_at_helper<Tuple, Action, I>{}(std::forward<Tuple>(tuple), std::forward<Action>(action));
        }
        else {
            tuple_apply_at_impl<std::index_sequence<Is...>>{}(std::forward<Tuple>(tuple), std::forward<Action>(action), index);
        }
    }
};
